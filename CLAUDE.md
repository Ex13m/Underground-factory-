...
3. Обновить `src/data/updates.ts` — 3-5 коротких строк «что поменялось»
   (RU/EN): их показывает бегущий анонс обновлений (fx/UpdatesTicker)
   один раз каждому посетителю после выхода версии.
...
Заявки админа: арт-редактор → «Заказать ▸ Higgsfield» → очередь в Админке → АРТ →
пользователь отдаёт `gen-queue.json` Claude → Claude генерирует и кладёт файлы,
пользователь применяет их кнопкой «Картинка файлом» (или Claude прописывает в seed).
